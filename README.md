...
- 前端顯示累計總瀏覽量，以及近 3 分鐘仍有活動的匿名在線分頁數
...
- `page_view`、匿名在線心跳、ZIP 接受／拒絕、模型開啟成功／失敗。
- STEP schema：AP203、AP214、AP242、Other 或 Unknown。
- 預先定義的錯誤分類，例如加密、ZIP64、解析失敗或沒有可顯示幾何。
- 以每日 salted hash 去重的訪客數，以及只存在分頁記憶體的匿名 session ID。後端只保存 session 的 salted hash 與最後活動秒數。

不記錄 ZIP、STEP、檔名、零件名稱、尺寸、幾何、來源 hash、原始 IP、User-Agent 或完整錯誤訊息。「目前在線」定義為近 3 分鐘送出心跳的匿名瀏覽器分頁；心跳不會增加總瀏覽量。daily visitor hash 與 event receipt 最多保留 180 天，過期的在線 session row 會由排程清除；每日 aggregate count 會持續保留。
...
5. 部署 Worker。正式環境把 Worker route 限制在既有網站的 `https://step3d-sim.petrichor.tw/api/analytics/*`，而 `analytics-config.js` 使用同網域的 `/api/analytics`；不需要額外的分析子網域，Workers.dev 與 preview URL 也維持關閉。

   ```bash
   npx wrangler@latest deploy
   ```

後台位於 `https://step3d-sim.petrichor.tw/api/analytics/admin`，輸入 `ADMIN_TOKEN` 後可看 7／30／90／180 天的訪客、瀏覽、成功／失敗與 schema 分布。token 會在送出請求後立即從輸入欄位清空，不會寫入瀏覽器儲存空間；正式環境的 token 存於 Cloudflare encrypted secret 與本機 macOS Keychain，不會進入 Git。
